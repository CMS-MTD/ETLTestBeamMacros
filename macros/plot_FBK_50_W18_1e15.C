...
{
	map_plotter mp;
	mp.tag = "FBK_50_W18_1e15_2x2"; //This defines names of all output files
	mp.sensor_name="FBK 2x2 50 W18 1e15 FNAL Board"; //Must be contained in sensor name as defined in AirTable. This determines which scope channels to consider.
	mp.chainPath = "root://cmseos.fnal.gov//store/group/cmstestbeam/SensorBeam2022/LecroyScope/RecoData/TimingDAQRECO/RecoWithTracks/v11/";
	mp.debug=false;

	//Define run range
	//Note: will try to load every run in this range, even if it doesn't exist (so, expect some harmless complaints.)
	mp.run_end = new vector<int>{61355};
	mp.run_start = new vector<int>{61186}; 



	//Define rotation angle and manual adjustments of x and y
	mp.angle = new vector<float>{0}; //degrees
	mp.x_offset= new vector<float>{3.34}; //mm
	mp.y_offset= new vector<float>{2.625}; //mm
	
	//v11: z_DUT -15, -12, -9, -6, -3, 0, 3, 6, 9, 12
	mp.dut_index = 5; //center of telescope
	mp.maxTrackChi2 = 10;
	mp.minTrackPix=4;
	mp.minTrackPlanes=9;
	...
	mp.hitThres=vector<float>(mp.npad+1,60.);
	mp.photekMin=200;
	mp.photekMax=350;
	...
	mp.zMinGain=110.; mp.zMaxGain=-1.;  // Gain means MPV amplitude in mV, here...
	// mp.zMinSigmat=0.01e-9; mp.zMaxSigmat=0.08e-9; 
	mp.zMinSigmat=10; mp.zMaxSigmat=50; 
	mp.zMinDeltat=220; mp.zMaxDeltat=-1;
