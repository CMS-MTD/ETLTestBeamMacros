...
{
	map_plotter mp;
	mp.tag = "FBK_5x5_group3"; //This defines names of all output files
	...
	// conf 590
	mp.run_end = new vector<int>{54970}; 
	mp.run_start = new vector<int>{54750}; //54364

	//Define rotation angle and manual adjustments of x and y
	mp.angle = new vector<float>{-0.75}; //degrees
	mp.x_offset= new vector<float>{-0.0025}; //mm
	mp.y_offset= new vector<float>{0.008}; //mm
	...
	mp.ySliceMin={-0.9, 0.4};
	mp.ySliceMax={-0.4, 1};
	mp.xSliceMin={2};
	mp.xSliceMax={2.5};
	mp.channels={20,1,2,3,4,5,26};
