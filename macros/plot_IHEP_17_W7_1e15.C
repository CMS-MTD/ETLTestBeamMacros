...
int main(int argc, char **argv)
{
	...
	mp.run_start = new vector<int>{61006};


	//Define rotation angle and manual adjustments of x and y
	mp.angle = new vector<float>{-1}; //degrees
	mp.x_offset= new vector<float>{3.38}; //mm
	mp.y_offset= new vector<float>{2.51}; //mm
	...
	mp.zMinGain=89.; mp.zMaxGain=-1.;  // Gain means MPV amplitude in mV, here...
	// mp.zMinSigmat=0.01e-9; mp.zMaxSigmat=0.08e-9; 
	mp.zMinSigmat=10; mp.zMaxSigmat=50; 
	mp.zMinDeltat=205; mp.zMaxDeltat=-1;
